...
export default function ClassesIndex({ classes, teachers, auth }) {
  ...
  function submit(e) {
    ...
    if (modal.mode === "create") {
      ...
      patch(route("classes.update", modal.schoolClass.id), {
  ...
  function destroy(schoolClass) {
    if (confirm(`Delete class "${schoolClass.name}"?`))
    ...
    <AuthenticatedLayout header="Classes">
      ...
      {classes.data.length === 0 ? (
        <EmptyState
          ...
          description="Create classes like &ldquo;JSS 1A&rdquo; to start assigning students, teachers and subjects."
        />
      ) : (
            ...
            <div key={c.id} className="app-card register-rule">
              <div className="flex items-start justify-between">
                ...
                {isAdmin && (
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={() => destroy(c)}
                      className="rounded-lg p-1.5 text-slate-500 hover:bg-coral-50 hover:text-coral-700"
                    >
                      <Trash2 size={15} />
                    </button>
                  </div>
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-slate-600">
                  {c.students_count ?? 0} student
                  {c.students_count > 1 ? "s" : ""}
                </span>
                <span className="font-mono text-xs text-slate-500">
                  {c.class_teacher?.user?.name ?? "No class teacher"}
                </span>
              </div>
              {canTakeAttendance && (
                <Link
