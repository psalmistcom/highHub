...
export default function AttendanceRegister({ schoolClass, date, subjectId, records }) {
    const recordMap = useMemo(() => {
        const map = {};
        records.forEach((r) => { map[r.student_profile_id] = r.status.value ?? r.status; });
        return map;
    }, [records]);
    ...
    function changeDateOrSubject(field, value) {
        const params = { date: form.date, subject_id: form.subject_id, ...(field ? { [field]: value } : {}) };
        router.get(route('attendance.show', schoolClass.id), params, { preserveState: false });
    }

    function setStatus(studentId, status) {
        setForm((f) => ({ ...f, statuses: { ...f.statuses, [studentId]: status } }));
    }

    function markAll(status) {
        setForm((f) => ({
            ...f,
            statuses: Object.fromEntries(schoolClass.students.map((s) => [s.id, status])),
        }));
    }
        ...
        <AuthenticatedLayout header={`Attendance · ${schoolClass.name}`}>
            ...
            <div className="app-card mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div className="flex flex-wrap gap-3">
                    <div>
                        ...
                        <input type="date" className="field-input" value={form.date}
                            onChange={(e) => { setForm((f) => ({ ...f, date: e.target.value })); changeDateOrSubject('date', e.target.value); }} />
                    </div>
                    <div>
                        <label className="field-label">Subject (optional — whole day if blank)</label>
                        <select className="field-input" value={form.subject_id}
                            onChange={(e) => { setForm((f) => ({ ...f, subject_id: e.target.value })); changeDateOrSubject('subject_id', e.target.value); }}>
                ...
                <div className="flex gap-2">
                    <button type="button" onClick={() => markAll('present')} className="btn-secondary text-xs">Mark all present</button>
                </div>
            </div>
